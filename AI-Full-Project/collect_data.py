...
def collect_training_data(total_actions):
    ...
    for action_i in range(total_actions):
        ...
        print(f'Collecting Training Data {progress}%   ')
    ...
    with open('saved/training_data.csv', 'w', newline='') as csvfile:
        spamwriter = csv.writer(csvfile, delimiter=',')
        for i in range(len(network_params[0])):
            list = [network_params[0][i][0],
                    network_params[0][i][1],
                    network_params[0][i][2],
                    network_params[0][i][3],
                    network_params[0][i][4],
                    network_params[1][i],
                    network_params[2][i]]
